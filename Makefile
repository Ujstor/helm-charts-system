...
CHARTS := cluster-issuer ingress-nginx external-secrets cert-manager cilium  argo-cd vault crunchy-postgres-operator crunchy-postgres-cluster rook-ceph-operator rook-ceph-cluster secret-store minio-operator minio-tenant gitlab-operator metallb-operator metallb-config prometheus-grafana istiod tekton-operator tekton-pipelines
